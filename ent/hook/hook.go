...
// The PersonalAccountFunc type is an adapter to allow the use of ordinary
// function as PersonalAccount mutator.
type PersonalAccountFunc func(context.Context, *ent.PersonalAccountMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PersonalAccountFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PersonalAccountMutation); ok {
		return f(ctx, mv)
	}
